...
var ASM_CONSTS = {
 ...
 1036229: ($0, $1) => { var SDL2 = Module['SDL2']; var buf = $0 >>> 2; var numChannels = SDL2.audio.currentOutputBuffer['numberOfChannels']; for (var c = 0; c < numChannels; ++c) { var channelData = SDL2.audio.currentOutputBuffer['getChannelData'](c); if (channelData.length != $1) { throw 'Web Audio output buffer length mismatch! Destination size: ' + channelData.length + ' samples vs expected ' + $1 + ' samples!'; } for (var j = 0; j < $1; ++j) { channelData[j] = HEAPF32[buf + (j*numChannels + c)]; } } },  
 1036718: ($0) => { var SDL2 = Module['SDL2']; if ($0) { if (SDL2.capture.silenceTimer !== undefined) { clearInterval(SDL2.capture.silenceTimer); } if (SDL2.capture.stream !== undefined) { var tracks = SDL2.capture.stream.getAudioTracks(); for (var i = 0; i < tracks.length; i++) { SDL2.capture.stream.removeTrack(tracks[i]); } } if (SDL2.capture.scriptProcessorNode !== undefined) { SDL2.capture.scriptProcessorNode.onaudioprocess = function(audioProcessingEvent) {}; SDL2.capture.scriptProcessorNode.disconnect(); } if (SDL2.capture.mediaStreamNode !== undefined) { SDL2.capture.mediaStreamNode.disconnect(); } SDL2.capture = undefined; } else { if (SDL2.audio.scriptProcessorNode != undefined) { SDL2.audio.scriptProcessorNode.disconnect(); } if (SDL2.audio.silenceTimer !== undefined) { clearInterval(SDL2.audio.silenceTimer); } SDL2.audio = undefined; } if ((SDL2.audioContext !== undefined) && (SDL2.audio === undefined) && (SDL2.capture === undefined)) { SDL2.audioContext.close(); SDL2.audioContext = undefined; } },  
 1037724: ($0, $1, $2) => { var w = $0; var h = $1; var pixels = $2; if (!Module['SDL2']) Module['SDL2'] = {}; var SDL2 = Module['SDL2']; if (SDL2.ctxCanvas !== Module['canvas']) { SDL2.ctx = Browser.createContext(Module['canvas'], false, true); SDL2.ctxCanvas = Module['canvas']; } if (SDL2.w !== w || SDL2.h !== h || SDL2.imageCtx !== SDL2.ctx) { SDL2.image = SDL2.ctx.createImageData(w, h); SDL2.w = w; SDL2.h = h; SDL2.imageCtx = SDL2.ctx; } var data = SDL2.image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = 0xff; src++; dst += 4; } } else { if (SDL2.data32Data !== data) { SDL2.data32 = new Int32Array(data.buffer); SDL2.data8 = new Uint8Array(data.buffer); SDL2.data32Data = data; } var data32 = SDL2.data32; num = data32.length; data32.set(HEAP32.subarray(src, src + num)); var data8 = SDL2.data8; var i = 3; var j = i + 4*num; if (num % 8 == 0) { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; data8[i] = 0xff; i = i + 4 | 0; } } else { while (i < j) { data8[i] = 0xff; i = i + 4 | 0; } } } SDL2.ctx.putImageData(SDL2.image, 0, 0); },  
 1039190: ($0, $1, $2, $3, $4) => { var w = $0; var h = $1; var hot_x = $2; var hot_y = $3; var pixels = $4; var canvas = document.createElement("canvas"); canvas.width = w; canvas.height = h; var ctx = canvas.getContext("2d"); var image = ctx.createImageData(w, h); var data = image.data; var src = pixels / 4; var dst = 0; var num; if (typeof CanvasPixelArray !== 'undefined' && data instanceof CanvasPixelArray) { num = data.length; while (dst < num) { var val = HEAP32[src]; data[dst ] = val & 0xff; data[dst+1] = (val >> 8) & 0xff; data[dst+2] = (val >> 16) & 0xff; data[dst+3] = (val >> 24) & 0xff; src++; dst += 4; } } else { var data32 = new Int32Array(data.buffer); num = data32.length; data32.set(HEAP32.subarray(src, src + num)); } ctx.putImageData(image, 0, 0); var url = hot_x === 0 && hot_y === 0 ? "url(" + canvas.toDataURL() + "), auto" : "url(" + canvas.toDataURL() + ") " + hot_x + " " + hot_y + ", auto"; var urlBuf = _malloc(url.length + 1); stringToUTF8(url, urlBuf, url.length + 1); return urlBuf; },  
 1040178: ($0) => { if (Module['canvas']) { Module['canvas'].style['cursor'] = UTF8ToString($0); } },  
 1040261: () => { if (Module['canvas']) { Module['canvas'].style['cursor'] = 'none'; } },  
 1040330: () => { return window.innerWidth; },  
 1040360: () => { return window.innerHeight; }
};
function sync_all_url_params(raw_keys_ptr) { var urlParams = new URLSearchParams(window.location.search); var rawKeys = UTF8ToString(raw_keys_ptr); var cleanKeys = rawKeys.replace(new RegExp("'", "g"), "").replace(new RegExp("\"", "g"), ""); var allowedKeys = cleanKeys.split(",").map(function(k) { return k.trim(); }).filter(function(k) { return k !== ""; }); if (allowedKeys.length > 0) { setTimeout(function() { urlParams.forEach(function(value, key) { if (allowedKeys.indexOf(key) !== -1) { if (window.pushData) window.pushData(key, value); } }); }, 2000); } }
function setup_push_api() { window.pushData = function(key, value) { if (typeof key !== 'string') return; var valStr = String(value); var keySize = lengthBytesUTF8(key) + 1; var keyPtr = _malloc(keySize); stringToUTF8(key, keyPtr, keySize); var valSize = lengthBytesUTF8(valStr) + 1; var valPtr = _malloc(valSize); stringToUTF8(valStr, valPtr, valSize); var updateFn = Module["_on_data_received"]; if (updateFn) { updateFn(keyPtr, valPtr); console.log("WASM PUSH [DATA]: " + key + " -> " + valStr); } else { console.error("WASM ERROR: _on_data_received not found."); } _free(keyPtr); _free(valPtr); }; window.onBlynkGOEvent = function(key, value) { console.log("%c 📥 [WASM Event] " + key + " :", "color: #3498db; font-weight: bold;", value); var event = new CustomEvent("blynkgo_data", { detail: { key: key, value: value } }); window.dispatchEvent(event); }; }
function setup_mqtt(default_broker_ptr,default_topic_ptr) { var urlParams = new URLSearchParams(window.location.search); var rawBroker = urlParams.get('mqtt_host') || UTF8ToString(default_broker_ptr); var broker = rawBroker.split("\"").join("").trim(); var rawTopic = urlParams.get('mqtt_topic') || UTF8ToString(default_topic_ptr); var topic = rawTopic.split("\"").join("").trim(); window.mqttPub = function(t, m, retain = false) { if (window.mqttClient && window.mqttClient.connected) { window.mqttClient.publish(t, String(m), { retain: !!retain, qos: 0 }); console.log('🚀 MQTT Pub (Sent to Broker) -> ', t, ':', m); } }; window.mqttSub = function(topic) { if (window.mqttClient && window.mqttClient.connected) { window.mqttClient.subscribe(topic, function(err) { if (!err) { console.log('%c 📥 MQTT Subscribed -> ' + topic, 'color: #f39c12; font-weight: bold;'); } else { console.error('MQTT Subscribe Error:', err); } }); } else { console.error('MQTT: Client not ready'); } }; window.mqttUnsub = function(topic) { if (window.mqttClient) { window.mqttClient.unsubscribe(topic); console.log('%c 📤 MQTT Unsubscribed -> ' + topic, 'color: #e74c3c; font-weight: bold;'); } }; window.setMqttTopic = function(newTopic) { if (!window.mqttClient) return console.error('MQTT: Client not ready'); if (window.currentMqttTopic) { console.log('MQTT: Unsubscribing from -> ' + window.currentMqttTopic); window.mqttClient.unsubscribe(window.currentMqttTopic); } window.currentMqttTopic = newTopic; window.mqttClient.subscribe(newTopic); console.log('MQTT: Switched to Topic -> ' + newTopic); }; window.setMqttBroker = function(newBroker) { if (window.mqttClient) window.mqttClient.end(); console.log('MQTT: Reconnecting to -> ' + newBroker); initMqtt(newBroker, window.currentMqttTopic); }; function initMqtt(targetBroker, targetTopic) { window.currentMqttTopic = targetTopic; const client = mqtt.connect(targetBroker, { connectTimeout: 5000, keepalive: 60 }); window.mqttClient = client; client.on('connect', function () { console.log('MQTT: Connected to ' + targetBroker); client.subscribe(window.currentMqttTopic); console.log('%c [MQTT Console Ready] use: mqttPub(topic, msg, retain)', 'color: #3498db; font-weight: bold;'); }); client.on('message', function (t, msg) { var payload = msg.toString(); try { var data = JSON.parse(payload); for (var key in data) { if (data.hasOwnProperty(key)) { if (window.pushData) window.pushData(key, String(data[key])); } } } catch (e) { var topicParts = t.split('/'); var key = topicParts[topicParts.length - 1]; if (window.pushData) window.pushData(key, payload); } }); client.on('error', function(err) { console.error('MQTT Error: ', err); }); client.on('close', function() { console.log('MQTT: Connection closed'); }); } if (typeof mqtt === 'undefined') { var script = document.createElement('script'); script.src = 'https://unpkg.com/mqtt/dist/mqtt.min.js'; script.onload = function() { initMqtt(broker, topic); }; document.head.appendChild(script); } else { initMqtt(broker, topic); } }
function get_web_dimensions(width,height,default_board_ptr,default_rot_ptr) { const urlParams = new URLSearchParams(window.location.search); let w = urlParams.get("w"); let h = urlParams.get("h"); let forceAuto = urlParams.has("auto_resize"); let board = urlParams.get("board") || UTF8ToString(default_board_ptr); let r = parseInt(urlParams.get("r") || UTF8ToString(default_rot_ptr)); board = board.split("\"").join(""); let finalW = 0, finalH = 0; if (w && h) { finalW = parseInt(w); finalH = parseInt(h); } else if (forceAuto) { finalW = window.innerWidth || document.documentElement.clientWidth; finalH = window.innerHeight || document.documentElement.clientHeight; } else if (board && board !== "" && board !== "undefined" && board !== "BOARD_NAME") { let bw = 800, bh = 480; if (board === "beenext_1_9") { bw = 320; bh = 170; } else if (["beenext_2_4c", "beenext_2_8r", "beenext_3_2c"].includes(board)) { bw = 320; bh = 240; } else if (["beenext_3_5c", "blynkgo_board_v1_3", "blynkgo_emmaII_4mb"].includes(board)) { bw = 480; bh = 320; } else if (board === "beenext_4_3c") { bw = 480; bh = 272; } else if (["beenext_4_3ips", "beenext_5_0ips", "beenext_7_0ips", "beenext_7_0"].includes(board)) { bw = 800; bh = 480; } if (r % 2 !== 0) { finalW = bh; finalH = bw; } else { finalW = bw; finalH = bh; } } else { finalW = window.innerWidth || document.documentElement.clientWidth; finalH = window.innerHeight || document.documentElement.clientHeight; } setValue(width, finalW, "i32"); setValue(height, finalH, "i32"); }
function setup_resize_listener(board_name_ptr) { window.addEventListener("resize", function() { const urlParams = new URLSearchParams(window.location.search); let forceAuto = urlParams.has("auto_resize"); let hasUrlBoard = urlParams.get("board"); let hasUrlSize = urlParams.get("w") && urlParams.get("h"); let boardName = UTF8ToString(board_name_ptr).split("\"").join(""); if (!forceAuto && (hasUrlBoard || hasUrlSize || (boardName !== "" && boardName !== "BOARD_NAME"))) return; let w = window.innerWidth || document.documentElement.clientWidth; let h = window.innerHeight || document.documentElement.clientHeight; if(typeof _on_browser_resize === "function") _on_browser_resize(w, h); }); }
function mqtt_publish_raw(topic,message,retain) { if (window.mqttPub) { var t = UTF8ToString(topic); var m = UTF8ToString(message); window.mqttPub(t, m, !!retain); } else { console.error("MQTT Error: window.mqttPub is not defined"); } }
function blynkgo_to_js(key,value) { var k = UTF8ToString(key); var v = UTF8ToString(value); var event = new CustomEvent("blynkgo_data", { detail: { key: k, value: v } }); window.dispatchEvent(event); console.log("%c 📤 [BlynkGO -> JS] " + k + " : " + v, "color: #e67e22; font-weight: bold;"); }
...
function assignWasmExports(wasmExports) {
  ...
  _on_browser_resize = Module['_on_browser_resize'] = wasmExports['on_browser_resize'];
  _main = Module['_main'] = wasmExports['__main_argc_argv'];
  _on_data_received = Module['_on_data_received'] = wasmExports['on_data_received'];
